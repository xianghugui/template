...
public class FileController {
    ...
    @AccessLogger("下载文件")
    public ResponseMessage restDownLoad(@PathVariable("id") Long id,
                                        @PathVariable("name") String name,
                                        HttpServletResponse response, HttpServletRequest request) throws Exception {
        return downLoad(id, name, response, request);
    }

    @RequestMapping(value = "/image/{resourceId}", method = RequestMethod.GET)
    @AccessLogger("图片资源加载")
    @Authorize(anonymous = true)
    public ResponseMessage loadImage(@PathVariable("resourceId") Long resourceId, HttpServletRequest request, HttpServletResponse response) throws IOException {
    ...
    public Map<String, Object> imgUpload(@RequestParam("file") MultipartFile file) throws IOException {
        ...
        if (!file.isEmpty()) {
            ...
            if (fileName.contains(".mp4")) {
                initialPreview = new String[]{"/file/download/" + resources.getId() + ".mp4"};
                initialPreviewConfig.put("type", "video");
                initialPreviewConfig.put("filetype", "video/mp4");
            } else {
                initialPreview = new String[]{"/file/image/" + resources.getId() + ".jpg"};

            }
            initialPreviewConfig.put("caption", resources.getName());
            initialPreviewConfig.put("url", "/shop/img/delete");
            initialPreviewConfig.put("key", String.valueOf(resources.getId()));
    ...
    @RequestMapping(value = "/imgCompressUpload", method = RequestMethod.POST)
    @AccessLogger("上传商品封面压缩图片")
    public Map<String, Object> imgCompressUpload(@RequestParam("file") MultipartFile file) throws IOException {
        if (logger.isInfoEnabled())
            logger.info("start upload.");
        Resources resources;
        HashMap<String, Object> result = new HashMap<>();
        if (!file.isEmpty()) {
            ...
            String fileName = file.getOriginalFilename();
            cacheManager.getCache("resources").clear();
            resources = fileService.saveCompressFile(file.getInputStream(), fileName);
            String error = "";
            String[] initialPreview;
            HashMap<String, String> initialPreviewConfig = new HashMap<>();
            initialPreview = new String[]{"/file/image/" + resources.getId() + ".jpg"};
