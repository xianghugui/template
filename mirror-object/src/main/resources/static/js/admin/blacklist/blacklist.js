$(function () {
            ...
            "columns": [
                {
                    ...
                    render: function (data, type, row, meta) {
                        var html = "<div class='img-show-box' class='btn btn-default' data-toggle='tooltip' data-placement='bottom' title='"
                            + data.createTime +"'><image class='img' src='" + data.imageUrl + "'></image>" +
                            "<div class='img-content'>" + data.name + "<button class='btn btn-default btn-xs btn-edit pull-right'>编辑</button></div>" +
    ...
    $("#black_list").off('click', '.btn-edit').on('click', '.btn-edit', function () {
        $(".modal-title").html("编辑");
        var data = black_list.row(this).data();
        black_id = data.id;
